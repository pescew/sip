...
var ErrInvalidRequest97 = fmt.Errorf("Invalid SIP %s request", fields.ReqACSResend.String())
...
func (ar *ACSResend) Marshal(delimiter, terminator rune, errorDetection bool) string {
	if errorDetection {
		...
		fmt.Fprintf(&msg, "%sAZ", fields.ReqACSResend.ID())
		msg.WriteString(utils.ComputeChecksum(msg.String()))
		msg.WriteRune(terminator)
		return msg.String()
	}
	return fmt.Sprintf("%s%c", fields.ReqACSResend.ID(), terminator)
}
...
func (ar *ACSResend) Unmarshal(line string, delimiter, terminator rune) error {
	...
	if string(runes[0:2]) != fields.ReqACSResend.ID() {
		return ErrInvalidRequest97
	}
...
func (ar *ACSResend) Validate() error {
	...
	if err != nil {
		return fmt.Errorf("invalid SIP %s did not pass validation: %v", fields.ReqACSResend.String(), err.(validator.ValidationErrors))
	}
	return nil
}
