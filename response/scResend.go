...
var ErrInvalidResponse96 = fmt.Errorf("Invalid SIP %s response", fields.RespSCResend.String())
...
func (scr *SCResend) Marshal(delimiter, terminator rune, errorDetection bool) string {
	if errorDetection {
		...
		fmt.Fprintf(&msg, "%sAZ", fields.RespSCResend.ID())
		msg.WriteString(utils.ComputeChecksum(msg.String()))
		msg.WriteRune(terminator)
		return msg.String()
	}
	return fmt.Sprintf("%s%c", fields.RespSCResend.ID(), terminator)
}
...
func (scr *SCResend) Unmarshal(line string, delimiter, terminator rune) error {
	...
	if string(runes[0:2]) != fields.RespSCResend.ID() {
		return ErrInvalidResponse96
	}
...
func (scr *SCResend) Validate() error {
	...
	if err != nil {
		return fmt.Errorf("invalid SIP %s did not pass validation: %v", fields.RespSCResend.String(), err.(validator.ValidationErrors))
	}
	return nil
}
